...
public class Item implements Runnable{
	...
	public void run() {
		...
		while(true)
		{
			...
			while(grid.GetTile(i, j).getType() == TileType.Vertical || grid.GetTile(i, j).getType() == TileType.Horizontal
		               || grid.GetTile(i, j).getType() == TileType.Cross)
		    {
		            i = random.nextInt(8) + 1;
		            j = random.nextInt(6) + 1;
		            System.out.println("horororo");
		    }
